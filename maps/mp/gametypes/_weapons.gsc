...
registerCvars()
{
	...
	[[var]]("scr_boltaction_nades", "INT", 2, 0, 99);
	[[var]]("scr_semiautomatic_nades", "INT", 1, 0, 99);
	[[var]]("scr_smg_nades", "INT", 1, 0, 99);
	[[var]]("scr_sniper_nades", "INT", 1, 0, 99);
	[[var]]("scr_mg_nades", "INT", 1, 0, 99);
...
GetGrenadeTypeName() // Adjust for cod1 grenades
{
	grenadetype = "none";
	if(self.pers["team"] == "allies")
	{
...
giveGrenadesFor(weapon, count) // Changed according to cod1 - equip with nades according the weapon type.
{
	// remove all grenades
	...
	if(fraggrenadecount)
	{
		...
		self setWeaponSlotClipAmmo("grenade", fraggrenadecount);
...
/* giveGrenadesFor(weapon, count) // COD2 
{
	// remove all grenades
...
/* dropWeapon() //Drop current weapon including pistol.
{
	...
	if (true == isPistol(current))
	{
		if (getCvarInt("scr_pistol_allow_drop") == 1)
		{
	...
	else if (true == isGrenade(current))
	{
		self dropItem(current);
		waittillframeend; // wait before next "weapon_dropped" can be called
		level notify("weapon_dropped", current, self);
...
dropCurrentWeapon() //Drop current weapon including pistol and grenade.
{
    current = self getCurrentWeapon();

    if (current == "none" || (level.gametype == "re" && self.obj_carrier))
        return;

    if (isPistol(current))
    {
        if (getCvarInt("scr_pistol_allow_drop") == 1)
            dropItemAndNotify(current);
    }
    else if (isGrenade(current) || canDropPrimaryWeapon(current))
    {
        dropItemAndNotify(current);
    }
}

canDropPrimaryWeapon(weapon)
{
    primaryWeapon = self getWeaponSlotWeapon("primary");
    primaryWeapon2 = self getWeaponSlotWeapon("primaryb");

    if (weapon == primaryWeapon || weapon == primaryWeapon2)
    {
...
dropItemAndNotify(weapon)
{
    self dropItem(weapon);
    waittillframeend;
    level notify("weapon_dropped", weapon, self);
	logPrint("Item dropped;" + weapon + ";" + self.name + "\n");
}
...
/* getWeaponBasedSmokeGrenadeCount(weapon) // COD2
{
	className = level.weapons[weapon].classname;
...
/* getFragGrenadeCount() //COD2
{
	// Because player can pickup nades also from enemy team, all grenade types are counted
	count = 0;
	count += self getammocount("fraggrenade_mp");
	count += self getammocount("mk1britishfrag_mp");
	count += self getammocount("rgd-33russianfrag_mp");
	count += self getammocount("stielhandgranate_mp");
	return count;
}

getSmokeGrenadeCount() //COD2
{
	// Because player can pickup nades also from enemy team, all grenade types are counted
	count = 0;
...
/* keepMaxAmmo()      // Seems like both not used
{
