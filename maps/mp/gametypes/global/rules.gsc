Init()
{
	...
	if (game["firstInit"] || overtime_changed)
	{
		...
		// This is because of fucking cod1 precache system
...
IsValidPAMMode(cvar, value_now, registerTime)
{
	...
	if (!isValid)
	{
		...
		if (!registerTime)
		{
			iprintln("^3CVAR change detected: ^2" + value_now + "^3 is not a valid value for cvar ^2" + cvar["name"]);
