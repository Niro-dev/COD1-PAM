...
init()
{
	...
	if(game["firstInit"])
	{
		...
		precacheString2("STRING_PAM_MUST_EXISTS_UNDER_MAIN", &"pk3 file ^9zpam333.pk3^7 must be installed in ^9main^7 folder."); // ZPAM_RENAME
		precacheString2("STRING_PAM_GETTING_PK3_FILES_ERROR", &"Error while getting loaded pk3 files. Make sure pk3 files does not contains spaces.");
		...
		// Help url
		precacheString2("STRING_GITHUB_URL", &"https://github.com/Niro-dev/COD1-PAM");
		precacheString2("STRING_GITHUB_URL_HELP", &"Please visit ^9https://github.com/Niro-dev/COD1-PAM ^7for install instructions.");
	}
...
CheckInstallation()
{
	...
	// Wait untill sv_referencedPaks is set
	// Dont know why, but sv_referencedPaks cvars is defined after 4 frames
	wait level.fps_multiplier * 0.3; // just to be sure - on some servers cvars are set twice with different value every time

	sv_referencedPaks = getCvar("sv_referencedPaks");
	sv_referencedPakNames = getCvar("sv_referencedPakNames");
	sv_pakNames = getCvar("sv_pakNames");
	sv_paks = getCvar("sv_paks");

	// Referenced contains only loaded pk3s, but full path!
	//"sv_referencedPaks" is: "-340018619 473749641 181429573 780394069 1101180720 1046874969 1053665859 -1652414412 " default: ""
	//"sv_referencedPakNames" is: "main/zpam320_alpha main/iw_15 main/iw_13 main/iw_08 main/iw_07 main/iw_06 main/iw_04" default: ""

	// This contains all available pk3 names
	//"sv_pakNames" is: "zpam320_alpha mp_burgundy_fix iw_15 iw_14 iw_13 iw_12 iw_11 iw_10 iw_09 iw_08 iw_07 iw_06 iw_05 iw_04 iw_03 iw_02 iw_01 iw_00" default: ""
	//"sv_paks" is: "530543226 960396763 181429573 -1449716526 780394069 -1333623355 -1980843666 1334775335 -621896007 1101180720 1046874969 1053665859 1842349204 -1652414412 1659111092 -1085686032 -2025394354 178615151 " default: ""
...
*/
	...
	{
		level thread printTextInLoop("Cvar /fs_game = '" + level.fs_game + "'. It needs to be empty", "Make sure PK3 files are installed in main folder, not in mod folder.");

		setError(game["STRING_PAM_FS_GAME"]);
		return;
	}

	// Convert pk3 names to array
	fullNumbersArray = splitString(sv_referencedPaks, " ");
	fullNameArray = splitString(sv_referencedPakNames, " ");



	// Number of items is not same - may be due to spaces in pk3 names!
	if (fullNumbersArray.size != fullNameArray.size)
	{
		level thread printTextInLoop("Diagnostics info: referencedpaks", sv_referencedPakNames, sv_referencedPakNames, fullNumbersArray.size + "!=" + fullNameArray.size);

		setError(game["STRING_PAM_GETTING_PK3_FILES_ERROR"]);
		return;
	}
/*
	/#
...
*/
	// Convert pk3 names to array
	numbersArray = splitString(sv_paks, " ");
	nameArray = splitString(sv_pakNames, " ");

	// Number of items is not same - may be due to spaces in pk3 names!
	if (numbersArray.size != nameArray.size)
	{
		level thread printTextInLoop("Diagnostics info: pk3s", sv_paks, sv_pakNames, numbersArray.size + "!=" + nameArray.size);
...
*/
	...
	{
		level thread printTextInLoop("Diagnostics info: sv_referencedPakNames does not contain " + level.pam_folder);

		setError(game["STRING_PAM_MUST_EXISTS_UNDER_MAIN"]);
		return;
	}

	/* if (getCvar("shortversion") == "1.5" && (getCvarInt("sv_wwwDownload") == 0 || getCvar("sv_wwwBaseURL") == ""))
	{
		setError(game["STRING_PAM_WWW_DOWNLOADING"]);
		return;
	}

	if (getCvar("shortversion") == "1.5" && !arrayContains(nameArray, level.pam_map_iwd))
	{
		//NEED TO REMOVE QTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
		setError(game["STRING_PAM_MAPS_MISSING"]);
		return;
	} */

	/* if (getCvar("shortversion") == "1.5")
	{
		maps = [];
