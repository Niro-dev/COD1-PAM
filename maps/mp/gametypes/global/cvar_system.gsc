...
limitCvar(cvar, registerTime)
{
	...
	if(getCvar(cvar["name"]) == "" && cvar["type"] != "STRING")
	{
		setCvar(cvar["name"], cvar["defaultValue"]);
		if (!registerTime)
			iprintln("^3CVAR change detected: ^2" + cvar["name"] + " ^3 was not defined");
		return false;
	}
	else if (cvar["type"] == "INT" || cvar["type"] == "FLOAT" || cvar["type"] == "BOOL") // string does not have min - max value
	{
		...
		if(isDefined(cvar["maxValue"]) && value_now > cvar["maxValue"])
		{
			...
			if (!registerTime) {
				iprintln("^3CVAR change detected: ^2" + value_now + "^3 is not a valid value for dvar ^2" + cvar["name"]);
				iprintln("^3CVAR change detected: Domain is a value from ^2"+cvar["minValue"]+"^3 to ^2"+cvar["maxValue"]);
			} else {
